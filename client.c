...
int main(int argc,char *argv[]) {
	...
	//check arguments
	if (argc != 4) {
        fprintf(stderr,"missing arguments, please use the form ./client <usrname> <IP_add> <Port_num>\n");
        exit(1);
    }
	...
	createJoinMessage(&join_message, username);
	...
	while(1) {
		...
		if(FD_ISSET(STDIN_FILENO,&input_descriptors)) {
			...
			createSendMessage(&output_message, username, input_buffer);
